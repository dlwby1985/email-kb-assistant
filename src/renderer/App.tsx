...
function App() {
    ...
    return (
      <div className="min-h-screen app-bg flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-asu-gold/20 border-t-asu-gold rounded-full animate-spin mx-auto mb-4" />
  ...
  return (
    <div className="h-screen flex app-bg">
      ...
      {saveToast && (
        <div className="fixed bottom-6 right-6 z-50 gm-toast p-4 max-w-sm animate-fade-in shadow-xl">
          <div className="flex items-start gap-3">
            <span className="text-asu-gold text-xl shrink-0">✓</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white mb-1">Saved successfully</p>
              {saveToast.threadFilePath && (
                <div className="flex items-center gap-1.5 mb-1">
                  <span className="text-xs text-white/50 truncate">{saveToast.threadFilePath.split(/[/\\]/).slice(-3).join(' › ')}</span>
                  <button
              ...
              {saveToast.templateFilePath && (
                <div className="flex items-center gap-1.5">
                  <span className="text-xs text-white/50 truncate">📋 {saveToast.templateFilePath.split(/[/\\]/).slice(-2).join(' › ')}</span>
                  <button
            ...
            <button onClick={() => setSaveToast(null)} className="text-white/40 hover:text-white shrink-0 text-base leading-none">×</button>
          </div>
        </div>
      )}
      ...
      {/* Draft saved toast */}
      {draftToast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 gm-toast text-sm px-4 py-2 shadow-lg animate-fade-in">
