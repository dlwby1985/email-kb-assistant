      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {contactMode === 'single' && (
          <label className="flex items-center gap-2 cursor-pointer group">
            <input
              ...
              className="w-3.5 h-3.5 accent-asu-gold cursor-pointer"
            />
        ...
        <div className="space-y-1">
          <label className="flex items-center gap-2 cursor-pointer group">
            ...
            <span className="text-xs text-white/60 group-hover:text-white/80 transition-colors">
              {t('compose.searchKB')}
              <span className="text-white/40 ml-1">{t('compose.searchKBHint')}</span>
            </span>
          </label>
          ...
          {useKB && (
            <div className="ml-5 text-xs">
              {isKBSearching ? (
                <span className="text-white/40">{t('compose.searching')}</span>
              ) : kbResults.length === 0 ? (
                <span className="text-white/40">{t('compose.noKBResults')}</span>
              ) : (
                <div>
                  <button
                    onClick={() => setKBExpanded((v) => !v)}
                    className="text-asu-gold hover:text-asu-gold/70 transition-colors"
                  >
                    {selectedCount} document{selectedCount !== 1 ? 's' : ''} matched
                    {totalTokens > 0 && <span className="text-white/40 ml-1">(~{totalTokens} tokens)</span>}
                    <span className="ml-1">{kbExpanded ? '▲' : '▼'}</span>
                  </button>

                  {kbExpanded && (
                    <div className="mt-1.5 space-y-1.5 border-l-2 border-white/15 pl-2">
                      {kbResults.map((r, i) => (
                        <label key={r.filePath} className="flex items-start gap-1.5 cursor-pointer">
                          <input
                            ...
                            className="mt-0.5 w-3 h-3 accent-asu-gold cursor-pointer shrink-0"
                          />
                          <div className="min-w-0">
                            <p className="text-white/70 font-medium truncate">{r.filename}</p>
                            <p className="text-white/40 leading-snug line-clamp-2">{r.snippet}</p>
                          </div>
                        </label>
                      ))}
                    </div>
