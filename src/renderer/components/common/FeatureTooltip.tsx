...
export default function FeatureTooltip({
  ...
  return (
    <div className="relative inline-block w-full">
      {children}
      <div
        className={`absolute ${positionClasses[position]} z-40 w-64 text-white text-xs rounded-lg shadow-lg px-3 py-2.5`}
        style={{
      ...
      >
        ...
        <button
          ...
          className="bg-asu-gold/20 hover:bg-asu-gold/30 text-asu-gold text-xs font-medium px-2 py-0.5 rounded transition-colors"
        >
          Got it
        </button>
        <button
          onClick={markDismissed}
          className="ml-2 text-white/40 hover:text-white text-xs transition-colors"
          title="Don't show again"
        >
          ✕
        </button>
        {position === 'bottom' && (
          <div className="absolute -top-1.5 left-4 w-3 h-3 rotate-45" style={{ background: 'rgba(30,10,20,0.95)' }} />
        )}
        {position === 'top' && (
          <div className="absolute -bottom-1.5 left-4 w-3 h-3 rotate-45" style={{ background: 'rgba(30,10,20,0.95)' }} />
        )}
      </div>
    </div>
  )
