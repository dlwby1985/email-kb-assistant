...
export default function ActionButtons({
  ...
  return (
    <div className="border-t border-white/10 px-6 py-3 flex items-center gap-2 shrink-0">
      {/* Copy */}
      <button
        onClick={onCopy}
      ...
      <button
        ...
        className={`text-xs px-3 py-1.5 rounded font-medium border transition-all duration-150 flex items-center gap-1
          ${isEditing
            ? 'border-asu-gold text-asu-gold bg-asu-gold/10'
            : 'border-white/15 text-white/70 hover:border-asu-gold hover:text-asu-gold'
          }`}
