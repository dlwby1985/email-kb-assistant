  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-5 py-3.5 border-b border-white/10 shrink-0">
        ...
        <p className="text-xs text-white/40 mt-0.5">Search all saved threads and conversations</p>
      </div>
      ...
      <div className="border-b border-white/10 shrink-0">
        ...
        <div className="flex items-center gap-1 px-5 pt-3 pb-1">
          <button
            ...
            className={`text-xs px-2.5 py-1 rounded-full font-medium transition-colors ${
              mode === 'keyword' ? 'bg-asu-gold text-black' : 'text-white/50 hover:text-white/70'
            }`}
          >
            Keyword
          </button>
          <button
            ...
            className={`text-xs px-2.5 py-1 rounded-full font-medium transition-colors ${
              mode === 'semantic' ? 'bg-asu-gold text-black' : 'text-white/50 hover:text-white/70'
            }`}
          >
            Semantic
          </button>
            ...
            <span className="ml-auto text-xs text-white/40">
              ...
              {embeddedCount === 0 && (
                <button
                  ...
                  disabled={indexing}
                  className="ml-2 text-asu-gold hover:underline disabled:opacity-50"
                >
        ...
        <div className="flex items-center gap-3 px-5 py-3">
          ...
          <input
            ...
            className="flex-1 text-base outline-none text-white/80 placeholder-white/40 bg-transparent"
          />
          {loading && (
            <div className="w-4 h-4 border-2 border-white/20 border-t-asu-gold rounded-full animate-spin shrink-0" />
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!query.trim() && (
          <div className="py-12 text-center">
            {mode === 'keyword' ? (
              <>
                <p className="text-sm text-white/40">Start typing to search all saved threads</p>
                <p className="text-xs text-white/30 mt-1">Searches subject, content, contacts, and tags</p>
              </>
            ) : (
              <>
                <p className="text-sm text-white/40">Describe the conversation you're looking for</p>
                ...
                {embeddedCount === 0 && (
                  <button
                    onClick={handleIndexAll}
                    disabled={indexing}
                    className="mt-3 btn-gold text-xs px-3 py-1.5 disabled:opacity-50"
                  >
        ...
        {semanticError && (
          <div className="py-6 px-5 text-center">
            <p className="text-sm text-red-400 mb-1">Semantic search failed</p>
            <p className="text-xs text-white/40">{semanticError}</p>
            <p className="text-xs text-white/40 mt-1">Check that DASHSCOPE_API_KEY is set in your .env</p>
          </div>
        )}
        ...
        {query.trim() && !loading && !semanticError && results.length === 0 && (
          <div className="py-12 text-center">
            ...
            {mode === 'semantic' && embeddedCount === 0 && (
              <p className="text-xs text-white/30 mt-1">Index your threads first (click "Index now" above)</p>
            )}
          </div>
        )}
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-semibold text-asu-gold truncate">
                  {contact?.name || result.contactSlug}
                </span>
                <span className="text-xs text-white/30">·</span>
                <span className="text-xs text-white/40 capitalize">{result.channel}</span>
                {result.tags?.length > 0 && (
                  <>
                    <span className="text-xs text-white/30">·</span>
                    <span className="text-xs text-white/40">{result.tags.slice(0, 2).join(', ')}</span>
                  </>
                )}
              ...
              {result.snippet && mode === 'keyword' && (
                <div
                  className="text-xs text-white/50 mt-0.5 line-clamp-2 [&_mark]:bg-asu-gold/50 [&_mark]:text-white/80 [&_mark]:rounded-sm [&_mark]:px-0.5"
                  dangerouslySetInnerHTML={{ __html: result.snippet }}
                />
              )}
              {result.snippet && mode === 'semantic' && (
                <div className="text-xs text-white/50 mt-0.5 line-clamp-2">{result.snippet}</div>
              )}
            </button>
          )
        })}
        ...
        {/* Result count footer */}
        {results.length > 0 && (
          <div className="px-5 py-2 bg-white/5 border-t border-white/10 text-xs text-white/40 text-right">
