...
const directionConfig: Record<string, { arrow: string; label: string; color: string }> = {
  'outgoing':      { arrow: '↗', label: 'Sent',              color: 'text-green-400' },
  'incoming-reply':{ arrow: '↙', label: 'Received & Replied', color: 'text-blue-400' },
  'incoming-only': { arrow: '↙', label: 'Received',           color: 'text-white/50' },
}
...
function ThreadDetail({ thread, contactSlug, onBack, onDeleted, onLoadToWorkspace }: ThreadDetailProps) {
  ...
  const dir = directionConfig[thread.direction] || { arrow: '→', label: thread.direction, color: 'text-white/50' }
  const ch = channelConfig[thread.channel] || { dot: 'bg-white/40', label: thread.channel }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-2 px-4 py-2.5 text-sm text-white/60 hover:bg-white/5 border-b border-white/10 shrink-0"
      >
        <span>←</span>
      ...
      <div className="px-4 py-3 bg-white/5 border-b border-white/10 shrink-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs text-white/50">{ch.label}</span>
          ...
          <div className="ml-auto flex items-center gap-3 shrink-0">
            {onLoadToWorkspace && content && !loading && (
              <button
                onClick={handleLoad}
                className="text-xs text-asu-gold hover:underline font-medium"
        ...
        {thread.subject && (
          <p className="text-sm font-semibold text-white/80 mb-0.5">{thread.subject}</p>
        )}
        <p className="text-xs text-white/40">{formatFullDate(thread.created_at)}</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {loading ? (
          <div className="flex items-center justify-center py-8">
            <div className="w-6 h-6 border-2 border-asu-gold/20 border-t-asu-gold rounded-full animate-spin" />
          </div>
        ) : content ? (
          <div className="prose prose-sm prose-invert max-w-none">
            {content.split('\n').map((line, i) => {
              if (/^## /.test(line)) {
                return <h3 key={i} className="text-xs font-semibold text-white/50 uppercase tracking-wider mt-4 mb-1 first:mt-0">{line.replace(/^## /, '')}</h3>
              }
              if (/^_\(.*\)_$/.test(line.trim())) {
                return <p key={i} className="text-xs text-white/40 italic mb-1">{line.replace(/^_\(/, '').replace(/\)_$/, '')}</p>
              }
              if (line.trim() === '') {
                return <div key={i} className="h-2" />
              }
              return <p key={i} className="text-sm text-white/70 leading-relaxed">{line}</p>
            })}
          </div>
        ) : (
...
function HistoryPanel({ isOpen, contact, onClose, onLoadToWorkspace }: HistoryPanelProps) {
  ...
  return (
    <>
      ...
      {/* Slide-out panel */}
      <div
        className={`fixed top-0 right-0 bottom-0 w-[420px] shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-in-out ${
      ...
      >
        {/* Panel header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.12] shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-asu-gold">
          ...
          <button
            onClick={onClose}
            className="text-white/40 hover:text-white/70 text-xl leading-none p-1"
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center py-12">
                <div className="w-7 h-7 border-2 border-asu-gold/20 border-t-asu-gold rounded-full animate-spin" />
              </div>
            ) : threads.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
                <p className="text-sm font-medium text-white/50">No threads yet</p>
                  ...
                  return (
                    <li key={thread.fileName || thread.created_at}>
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`w-2 h-2 rounded-full ${ch.dot} shrink-0`} />
                          <span className="text-xs text-white/40">{ch.label}</span>
                          <span className={`text-xs ${dir.color}`}>{dir.arrow} {dir.label}</span>
                          <span className="ml-auto text-xs text-white/40 shrink-0">
                            {formatDate(thread.created_at)}
                          </span>
                        </div>
                        <p className="text-sm font-medium text-white/80 group-hover:text-asu-gold transition-colors truncate">
                        ...
                        {thread.tags && thread.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            ...
                            {thread.tags.length > 3 && (
                              <span className="text-xs text-white/40">+{thread.tags.length - 3}</span>
                            )}
                          </div>
                        )}
                      </button>
