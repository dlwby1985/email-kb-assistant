...
export default function WritingStylePage() {
  ...
  if (loadingList) {
    return <div className="p-6 text-sm text-white/40">Loading writing styles...</div>
  }
    ...
    <div className="flex h-full min-h-0 gap-0">
      ...
      <div className="w-44 shrink-0 border-r border-white/10 flex flex-col">
        <div className="px-3 pt-4 pb-2 flex items-center justify-between">
          <span className="text-xs font-semibold text-asu-gold uppercase tracking-wide">My Styles</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-0.5">
          {styles.map((s) => (
            ...
            >
              {s.isDefault && (
                <span className={`text-xs ${s.slug === selectedSlug ? 'text-black/60' : 'text-yellow-500'}`}>★</span>
              )}
        ...
        <div className="p-2 border-t border-white/10">
          {showNewForm ? (
            ...
            <button
              ...
              className="w-full text-xs text-asu-gold font-medium hover:underline text-left px-1 py-1"
            >
              + New Style
            </button>
          )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {!selectedStyleInfo ? (
          <div className="p-6 text-sm text-white/40">Select or create a writing style.</div>
        ) : (
          <>
            {/* Header */}
            <div className="px-5 pt-4 pb-0 flex items-center justify-between shrink-0">
              <div>
                <h3 className="text-sm font-semibold text-asu-gold">
                ...
                {selectedStyleInfo.description && (
                  <p className="text-xs text-white/40 mt-0.5">{selectedStyleInfo.description}</p>
                )}
              </div>
              <div className="flex items-center gap-2">
                {!selectedStyleInfo.isDefault && (
                  <button
                    ...
                    className="text-xs text-white/50 hover:text-yellow-400 transition-colors disabled:opacity-40"
            ...
            {/* Tabs */}
            <div className="px-5 mt-3 flex gap-4 border-b border-white/10 shrink-0">
              ...
              <div className="flex-1 overflow-y-auto p-5">
                ...
                {activeTab === 'rules' && (
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {savedRules && <span className="text-xs text-green-400 font-medium">✓ Saved</span>}
                    ...
                    {(profile.analyzedPatterns && !analyzeResult) && (
                      <div className="mt-4">
                        <h4 className="text-xs font-semibold text-asu-gold mb-1">Analyzed Style Patterns</h4>
                        <p className="text-xs text-white/40 mb-2">Auto-generated from your example emails.</p>
                        <div className="glass-card-inner p-3 text-xs text-white/70 whitespace-pre-wrap max-h-48 overflow-y-auto">
                    ...
                    {analyzeResult && (
                      <div className="mt-4">
                        <span className="text-green-400 font-medium text-sm">✓ Style Analysis Complete</span>
                        <div className="mt-1 bg-green-500/10 border border-green-500/30 rounded p-3 text-xs text-white/70 whitespace-pre-wrap max-h-48 overflow-y-auto">
                          {analyzeResult}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-white/40">
                        ...
                        <span className={`font-medium ${canAnalyze ? 'text-green-400' : 'text-amber-400'}`}>
                          {examples.length} / 3 minimum
                        </span>
                      </p>
                      <button
                        onClick={() => setShowAddForm((v) => !v)}
                        className="text-xs text-asu-gold font-medium hover:underline shrink-0"
                      >
                    ...
                    {/* Add example form */}
                    {showAddForm && (
                      <div className="border border-asu-gold/30 rounded p-4 bg-asu-gold/5 space-y-3">
                        <h4 className="text-xs font-semibold text-asu-gold uppercase tracking-wide">New Example</h4>
                        <div>
                          <label className="text-xs font-medium text-white/60 block mb-1">Context *</label>
                          <input
                            type="text"
                        ...
                        <div className="flex gap-3">
                          <div className="flex-1">
                            <label className="text-xs font-medium text-white/60 block mb-1">Channel</label>
                            <select
                        ...
                        <div>
                          <label className="text-xs font-medium text-white/60 block mb-1">Email Content *</label>
                          <textarea
                    ...
                    {/* Example list */}
                    {examples.length === 0 ? (
                      <p className="text-xs text-white/40 italic">No examples yet. Add some emails above.</p>
                    ) : (
                      <div className="space-y-2">
                        {examples.map((ex) => (
                          <div
                            key={ex.fileName}
                            className="glass-card-inner rounded px-3 py-2 flex items-start justify-between gap-3"
                          >
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="text-sm font-medium text-white/80">{ex.context || 'Untitled'}</span>
                                <span className="text-xs bg-white/5 text-white/50 px-1.5 py-0.5 rounded capitalize">{ex.channel}</span>
                                <span className="text-xs bg-white/5 text-white/50 px-1.5 py-0.5 rounded capitalize">{ex.language}</span>
                              </div>
                              <p className="text-xs text-white/40 mt-1 truncate">{ex.contentPreview}…</p>
                            </div>
                            <button
                              onClick={() => handleDeleteExample(ex.fileName)}
                              className="text-white/30 hover:text-red-400 text-xs transition-colors shrink-0 mt-0.5"
                            >✕</button>
                          </div>
                        ))}
                      </div>
                    ...
                    {/* Analyze button */}
                    <div className="pt-2">
                      {analyzeError && <p className="text-xs text-red-400 mb-2">{analyzeError}</p>}
                      <button
                        onClick={handleAnalyze}
                        disabled={!canAnalyze || isAnalyzing}
                        className="btn-gold text-sm disabled:opacity-40 flex items-center gap-2"
                      >
                        {isAnalyzing ? (
                          <>
                ...
                {activeTab === 'auto' && (
                  <div className="space-y-5 max-w-md">
                    ...
                    <div>
                      <h4 className="text-xs font-semibold text-asu-gold mb-2 uppercase tracking-wide">Contact Relationship</h4>
                        ...
                        {RELATIONSHIPS.map((r) => (
                          <label key={r.value} className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              className="rounded border-white/20 text-asu-gold focus:ring-asu-gold"
                            />
                    ...
                    <div>
                      <h4 className="text-xs font-semibold text-asu-gold mb-2 uppercase tracking-wide">Channel</h4>
                      <div className="space-y-1.5">
                        {CHANNELS.map((c) => (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {savedAuto && <span className="text-xs text-green-400 font-medium">✓ Saved</span>}
                    </div>
                  </div>
                )}
