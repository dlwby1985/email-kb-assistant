...
const sections: Section[] = [
  {
    ...
    content: (
      <div className="space-y-3 text-sm text-white/70">
        <div>
          <p className="font-medium text-white/80 mb-1">System Requirements</p>
        ...
        <div>
          <p className="font-medium text-white/80 mb-1">First Launch</p>
          <ol className="list-decimal list-inside space-y-0.5 text-white/60 ml-2">
        ...
        <div>
          <p className="font-medium text-white/80 mb-1">Your First Email</p>
          <ol className="list-decimal list-inside space-y-0.5 text-white/60 ml-2">
            <li>Click the <strong className="text-white/80">Compose</strong> tab</li>
            ...
            <li>Click <strong className="text-white/80">Generate</strong></li>
      ...
      <div className="space-y-2 text-sm text-white/70">
        {[
          ...
          <div key={term} className="flex gap-2">
            <span className="font-medium text-white/80 w-32 shrink-0">{term}</span>
    ...
    content: (
      <div className="space-y-2 text-sm text-white/70">
        ...
        <ul className="list-disc list-inside space-y-0.5 text-white/60 ml-2">
          ...
          <li>Click <strong className="text-white/80">History</strong> to view past communication threads with a contact</li>
        </ul>
      </div>
    ),
  },
    ...
    content: (
      <div className="space-y-2 text-sm text-white/70">
        ...
        <ol className="list-decimal list-inside space-y-0.5 text-white/60 ml-2">
          ...
          <li>Click <strong className="text-white/80">Save Settings</strong>, then <strong className="text-white/80">Test Connection</strong></li>
        ...
        <p className="text-xs text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded px-2 py-1 mt-1">
    ...
    content: (
      <div className="space-y-2 text-sm text-white/70">
        ...
        <ol className="list-decimal list-inside space-y-0.5 text-white/60 ml-2">
          ...
          <li>Click <strong className="text-white/80">Analyze Examples</strong> to extract patterns automatically</li>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-2 text-sm text-white/70">
        ...
        <ul className="list-disc list-inside space-y-0.5 text-white/60 ml-2">
          <li><strong className="text-white/80">Keyword</strong>: Traditional full-text search — exact word matches</li>
          <li><strong className="text-white/80">Semantic</strong>: Meaning-based search using AI embeddings — finds related content even without exact word matches</li>
        </ul>
        <p className="text-white/50">Tip: Use Ctrl+K from anywhere to jump to Search.</p>
      </div>
    ),
  },
  {
        ...
        <tbody>
          {[
            ...
            <tr key={key} className="border-b border-white/10 last:border-0">
              ...
              <td className="py-1.5 text-white/60">{action}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3 text-sm text-white/70">
        ...
        <div>
          <p className="font-medium text-white/80 mb-1">Windows Built-in (free)</p>
          <ul className="list-disc list-inside space-y-0.5 text-white/60 ml-2">
            <li>Press <kbd className="bg-white/5 border border-white/10 rounded px-1 py-0.5 text-xs font-mono">Win + H</kbd> to activate Windows Voice Typing</li>
            <li>Works in any text field — supports English and many other languages</li>
          </ul>
        </div>
        <div>
          <p className="font-medium text-white/80 mb-1">Third-Party Tools</p>
          <ul className="list-disc list-inside space-y-0.5 text-white/60 ml-2">
            <li><strong className="text-white/80">Buzz</strong> (github.com/chidiwilliams/buzz) — free, offline Whisper-based transcription</li>
            <li><strong className="text-white/80">WhisperWriter</strong> — always-on voice typing using Whisper</li>
            <li><strong className="text-white/80">Otter.ai</strong> — cloud-based transcription with speaker identification</li>
          </ul>
        </div>
        <div>
          <p className="font-medium text-white/80 mb-1">Tips</p>
          <ul className="list-disc list-inside space-y-0.5 text-white/60 ml-2">
      ...
      <div className="space-y-2 text-sm text-white/70">
        {[
          ...
          <div key={problem} className="flex gap-2">
            <span className="font-medium text-white/80 w-36 shrink-0">• {problem}</span>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
        {filteredSections.length === 0 && (
          <p className="text-sm text-white/40 italic text-center py-8">No topics match "{search}"</p>
        )}
        {filteredSections.map((section) => {
          ...
          return (
            <div key={section.id} className="glass-card overflow-hidden">
              ...
              >
                <span className="text-sm font-semibold text-asu-gold">{section.title}</span>
