...
const tierBadge: Record<ModelOption['tier'], { label: string; className: string }> = {
  expensive: { label: 'High cost', className: 'bg-red-500/20 text-red-400' },
  balanced:  { label: 'Balanced',  className: 'bg-asu-gold/20 text-asu-gold' },
  cheap:     { label: 'Low cost',  className: 'bg-green-500/20 text-green-400' },
}
...
function GeneralSection({ config, onSave }: { config: Config | null; onSave: (cfg: Config) => Promise<void> }) {
  ...
  if (!config) {
    return <div className="flex items-center justify-center h-32 text-sm text-white/40">No config loaded</div>
  }
  ...
  return (
    <div className="p-6 max-w-xl space-y-6">
      ...
      {/* Interface Language */}
      <div>
        <h3 className="text-sm font-semibold text-white/80 mb-1">{t('settings.language')}</h3>
          ...
          {([
            ...
            <button
              ...
              className={`px-4 py-2 rounded text-sm font-medium border transition-colors ${
                uiLanguage === code
                  ? 'bg-asu-gold text-black border-asu-gold'
                  : 'text-white/70 border-white/10 hover:border-asu-gold'
              }`}
            >
              {label}
            </button>
      ...
      <div>
        <h3 className="text-sm font-semibold text-white/80 mb-3">{t('settings.llmProvider')}</h3>
          ...
          {providerOptions.map((opt) => (
            <label
              ...
              className={`flex items-start gap-3 px-3 py-2.5 rounded border cursor-pointer transition-colors ${
                ...
                provider === opt.id ? 'border-asu-gold bg-asu-gold/10' : 'border-white/10 hover:border-white/20'
              }`}
            >
              <input
                type="radio"
                ...
                className="text-asu-gold focus:ring-asu-gold shrink-0 mt-0.5"
              />
      ...
      {/* Claude config */}
      {provider === 'claude' && (
        <div className="space-y-4 border-l-2 border-asu-gold/20 pl-4">
          <div>
            <label className="text-xs font-medium text-white/60 block mb-1">
              Anthropic API Key
              {config.has_anthropic_key && (
                <span className="ml-2 text-green-400 font-normal">✓ key is set</span>
              )}
            </label>
            <input
              type="password"
            ...
            <p className="text-xs text-white/40 mt-1">
              Get a key at <span className="text-asu-gold">console.anthropic.com</span>
              {' · '}Keys are stored encrypted in OS keychain, not in config files.
            </p>
          </div>
          <div>
            <label className="text-xs font-medium text-white/60 block mb-2">Model</label>
            <div className="space-y-1.5 mb-2">
                ...
                return (
                  <label
                    ...
                    className={`flex items-center gap-3 px-3 py-2 rounded border cursor-pointer transition-colors ${
                      !useCustomModel && claudeModel === opt.id
                        ? 'border-asu-gold bg-asu-gold/10'
                  ...
                  >
                    <input
                      ...
                      className="text-asu-gold focus:ring-asu-gold shrink-0"
                    />
                    <div className="flex-1 min-w-0">
              ...
              <label
                className={`flex items-center gap-3 px-3 py-2 rounded border cursor-pointer transition-colors ${
                  useCustomModel ? 'border-asu-gold bg-asu-gold/10' : 'border-white/10 hover:border-white/20'
                }`}
              >
                <input
                  type="radio"
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-white">Custom model ID</p>
                  <p className="text-xs text-white/40">Enter any valid Anthropic model identifier</p>
                </div>
              </label>
            </div>
            {useCustomModel && (
      ...
      {provider === 'openai' && (
        <div className="space-y-3 border-l-2 border-asu-gold/20 pl-4">
          <div>
            <label className="text-xs font-medium text-white/60 block mb-1">Provider Preset</label>
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded border font-medium transition-colors ${
                    openaiPresetName === p.name
                      ? 'border-asu-gold bg-asu-gold text-black'
                      : 'border-white/10 text-white/60 hover:border-asu-gold hover:text-asu-gold'
                  }`}
                >
                  {p.name}
                </button>
          ...
          <div>
            <label className="text-xs font-medium text-white/60 block mb-1">Base URL</label>
            <input
              type="text"
              value={openaiBaseUrl}
          ...
          <div>
            ...
            <input
              ...
              className="input-field text-sm w-full font-mono"
            />
            <p className="text-xs text-white/40 mt-1">Keys stored encrypted in OS keychain.</p>
          </div>
          <div>
            <label className="text-xs font-medium text-white/60 block mb-1">Model</label>
            <input
              type="text"
              value={openaiModel}
      ...
      {provider === 'ollama' && (
        <div className="space-y-3 border-l-2 border-asu-gold/20 pl-4">
          <p className="text-xs text-white/50">
            Install Ollama from <span className="text-asu-gold font-medium">ollama.com</span>, then run{' '}
            <code className="bg-white/5 px-1 rounded text-xs">ollama serve</code> and pull a model with{' '}
            <code className="bg-white/5 px-1 rounded text-xs">ollama pull llama3.1</code>.
          </p>
          <div>
            <label className="text-xs font-medium text-white/60 block mb-1">Ollama URL</label>
            <input
              type="text"
              value={ollamaBaseUrl}
          ...
          <div>
            <label className="text-xs font-medium text-white/60 block mb-1">Model Name</label>
            <input
              type="text"
              value={ollamaModel}
            ...
            />
            <p className="text-xs text-white/40 mt-1">
              For bilingual (Chinese/English): try <code className="bg-white/5 px-1 rounded">qwen2.5:7b</code>
            </p>
          </div>
        </div>
      )}
      ...
      <div>
        <h3 className="text-sm font-semibold text-white/80 mb-1">
          Max Context Threads <span className="font-normal text-white/40">(1–20)</span>
        </h3>
        <p className="text-xs text-white/40 mb-2">How many past threads are loaded as context for each generation.</p>
        <div className="flex items-center gap-3">
          ...
          />
          <span className="w-8 text-center text-sm font-semibold text-white/70">{maxContextThreads}</span>
        </div>
      </div>
      ...
      {config.vault_path && (
        <div>
          <h3 className="text-sm font-semibold text-white/80 mb-1">Data Storage</h3>
          <div className="glass-card p-3 text-xs text-white/60 space-y-1">
            {config.storage_mode === 'standalone' ? (
              <p>Data stored in: <span className="font-mono text-white/70 break-all">{config.vault_path}</span></p>
            ) : (
              <p>Data stored in Obsidian vault: <span className="font-mono text-white/70 break-all">{config.vault_path}</span></p>
            )}
            <p className="text-white/40">EmailKB/ subfolder · config.yaml · contacts/ · skills/ · templates/</p>
          </div>
        </div>
      )}
...
function ShortcutsSection() {
  return (
    <div className="p-6 max-w-md">
      <h3 className="section-heading mb-4">Keyboard Shortcuts</h3>
      <table className="w-full text-sm">
        <tbody>
          {SHORTCUTS.map(({ key, action }) => (
            <tr key={key} className="border-b border-white/10 last:border-0">
              <td className="py-2 pr-4 w-40">
                <kbd className="bg-white/5 border border-white/10 rounded px-1.5 py-0.5 text-xs font-mono text-white/70 whitespace-nowrap">
                  {key}
                </kbd>
              </td>
              <td className="py-2 text-white/60">{action}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function AboutSection({ onOpenHelp }: { onOpenHelp?: () => void }) {
  return (
    <div className="p-6 max-w-sm">
      <div className="mb-6">
        <h3 className="text-base font-semibold text-asu-gold">Email KB Assistant</h3>
        <p className="text-xs text-white/40 mt-1">AI-powered email drafting and knowledge base</p>
      </div>
      <dl className="space-y-2 text-sm mb-6">
        <div className="flex gap-3">
          <dt className="text-white/40 w-24 shrink-0">Version</dt>
          <dd className="text-white/70">1.1.0 — Built {typeof __BUILD_DATE__ !== 'undefined' ? __BUILD_DATE__ : 'dev'}</dd>
        </div>
        <div className="flex gap-3">
          <dt className="text-white/40 w-24 shrink-0">Built by</dt>
          <dd className="text-white/70">Email KB Assistant Project</dd>
        </div>
        <div className="flex gap-3">
          <dt className="text-white/40 w-24 shrink-0">AI Models</dt>
          <dd className="text-white/70">Claude (Anthropic), DashScope embeddings</dd>
        </div>
      </dl>
      {onOpenHelp && (
...
function SignaturesSection({
  ...
  if (!config) return <div className="p-6 text-sm text-white/40">Loading...</div>

  return (
    <div className="p-6 max-w-2xl space-y-6">
      <div>
        <h3 className="section-heading mb-1">Email Signatures</h3>
        ...
        <div className="flex gap-4">
            ...
            {sigs.map((s) => (
              <button
                ...
                className={`w-full text-left px-2.5 py-2 rounded text-sm transition-colors flex items-center gap-1.5
                  ${s.id === selectedId
                    ? 'bg-asu-gold text-black font-medium'
                    : 'text-white/70 hover:bg-white/5'
                  }`}
              >
                {s.is_default && <span className={`text-xs ${s.id === selectedId ? 'text-black/60' : 'text-yellow-500'}`}>★</span>}
                <span className="truncate">{s.name}</span>
              </button>
            ))}
            <div className="pt-1">
              {showNew ? (
                <div className="space-y-1.5 glass-card-inner p-2">
                  <input
                    type="text"
                  ...
                  <div className="flex gap-1">
                    <button
                      ...
                      className="btn-gold text-xs px-2 py-1 flex-1 disabled:opacity-40"
                    >
                      {isCreating ? '...' : 'Add'}
                    </button>
                    <button
                ...
                <button
                  ...
                  className="w-full text-xs text-asu-gold font-medium hover:underline text-left px-1"
                >
                  + New Signature
                </button>
              )}
          ...
          {selectedSig && (
            <div className="flex-1 min-w-0 space-y-3">
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="text-xs font-medium text-white/60 block mb-1">Name</label>
                  <input
                    type="text"
                    value={editName}
                ...
                <div className="w-32">
                  <label className="text-xs font-medium text-white/60 block mb-1">Language</label>
                  <select
                    value={editLang}
              ...
              <div>
                <label className="text-xs font-medium text-white/60 block mb-1">Content</label>
                <textarea
                  value={editContent}
              ...
              {/* Preview */}
              {editContent && (
                <div className="glass-card-inner px-3 py-2">
                  <p className="text-xs text-white/40 mb-1 font-medium">Preview</p>
                  <pre className="text-xs text-white/70 whitespace-pre-wrap font-sans">{editContent}</pre>
                </div>
              )}
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {contentChanged && !isSaving && <span className="text-xs text-amber-400">Unsaved changes</span>}
                ...
                {sigs.length > 1 && (
                  <button
                    ...
                    className="text-xs text-white/30 hover:text-red-400 transition-colors ml-auto"
...
export default function SettingsPage({ config, onSave, onOpenHelp }: SettingsPageProps) {
  ...
  return (
    <div className="h-full flex">
      {/* Sidebar */}
      <div className="w-44 border-r border-white/10 shrink-0 flex flex-col pt-2">
        {SECTIONS.map((s) => (
          <button
            key={s.id}
            ...
            className={`text-left px-4 py-2.5 text-sm flex items-center gap-2 transition-colors ${
              activeSection === s.id
                ? 'bg-asu-gold/10 text-asu-gold font-medium border-r-2 border-asu-gold'
